...
void MysqlCon::InitTable(const std::string &msg_name)
{
	try {
		...
		string sql_str;
		if (!ProtoUtil::BuildCreateTableSql(msg_name, sql_str))
		{
...
bool MysqlCon::Insert(const db::ReqInsertData &req)
{
	...
	try {
		...
		string sql_str;
		ProtoUtil::CreateInsertSql(*msg, sql_str);
...
bool MysqlCon::Get(const db::ReqGetData &req)
{
	if (nullptr == m_con)
	{
		L_ERROR("nullptr == m_con");
...
bool MysqlCon::Del(const db::ReqDelData &req)
{
	if (nullptr == m_con)
	{
		L_ERROR("nullptr == m_con");
